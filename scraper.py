####################################
###This scraper works with in the scraperwiki
###IDE and extracts data contained in UN
###contriubtion documents. Set the url to the
###date you wish to extract. Upon completion
###the scraperwiki data store can export a
###SQLite database for use with the python
###script contained in sqlite_csv.py.
####################################
...
### Commented out above line "prev_mon = int(time.strftime("%m")) - 1" since it's January 2016 now, and the number would return -1. Restore the line above and delete the line below in February 2016.
prev_mon = int(12)
...
def Main():
    ...
    for page in list(root): #iterate through pages
        rtblocks = [ ] #initializes contribution blocks
        ...
        for text in page: #iterates through each text element
            ...
            if 130 <= int(text.attrib.get("left")) <= 140: # IDs contributor blocks...if not, moves on
                ...
                if textContent(text).strip() in tccReplace:
                    currentCountry = tccReplace[textContent(text).strip()]
                else:
                    currentCountry = textContent(text).strip()
            ...
            if 276 <= int(text.attrib.get("left")) <= 280: # IDs mission blocks...if not, moves on
                ...
                contributionData = {"date":date, "dateString":dateString, "tcc":currentCountry, "tccIso3Alpha":currentCountryISO3Alpha, "tccIso3Num":currentCountryISO3Num, "mission":currentMission}
                rtblocks = [ ]
        ...
        if rtblocks and contributionData:
            lndata = parseMissionBlock(rtblocks, contributionData)
            ldata.extend(lndata)
    scraperwiki.sqlite.save(["dateString", "tccIso3Alpha", "mission", "type"], ldata)
...
Ldescs = ['Contingent Troop', 'Experts on Mission', 'Formed Police Units', 'Individual Police','Military Observer', 'Troop', 'Police', 'Civilian Police' ]
...
def parseMissionBlock(rtblocks, data):
    ...
    for d in descs:
        ...
        elif len(drow) != 4 and data['tccIso3Alpha'] == 'PLW':
            ndata = None
    #     elif len(drow) == 1:
